...
class CreateTwistChain(bpy.types.Operator):
    ...
    bl_description = "Create a series of bones from one bone tail to a bone head that twist gradually from the start and end bone"
    bl_options = {'REGISTER', 'UNDO'}

    num_bones = IntProperty(name="Number", description="Number of bones in the chain", default=2, min=1)
    prefix = StringProperty(name="Prefix", description="Prefix to give the twist bones", default="Twist")
    suffix = StringProperty(name="Suffix", description="Suffix to give the twist bones", default="")
    num_executions = 0

    @classmethod
    def poll(cls, context):
        return bpy.context.object and bpy.context.object.type == 'ARMATURE' and bpy.context.object.select and bpy.context.mode == 'EDIT_ARMATURE' and len(
            context.selected_bones) == 2
